  ...
  try {
    ...
    const agentStates = await Promise.all(
      agents.map(async (agent) => {
        ...
        const tradesCompleted = trades.filter(
          t => t.fromAgentId === agent.id || t.toAgentId === agent.id
        ).length;
...
apiRouter.get('/proposals/:sessionId', async (req, res) => {
  try {
    ...
    // Enrich with details
    const enriched = await Promise.all(
      sessionProposals.map(async (proposal) => {
        const fromAgent = await db.query.agents.findFirst({
  ...
  try {
    ...
    const comparisonData = await Promise.all(
      agents.map(async (agent) => {
        ...
        const agentLeaderboardEntries = allLeaderboardEntries.filter(
          (entry) => entry.agentId === agent.id
        );

        const totalWealth = agentLeaderboardEntries.reduce(
          (sum, entry) => sum + entry.totalWealth,
          0
        );
        const sessionsPlayed = agentLeaderboardEntries.length;
        const wins = agentLeaderboardEntries.filter((entry) => entry.rank === 1).length;
        const totalTrades = agentLeaderboardEntries.reduce(
          (sum, entry) => sum + entry.tradesCompleted,
          0
        );
        ...
        if (sessionId) {
          ...
          const tradesCompleted = sessionTrades.filter(
            (t) => t.fromAgentId === agent.id || t.toAgentId === agent.id
          ).length;
        ...
        const headToHead: Record<string, any> = {};
        for (const otherAgent of agents) {
          if (!otherAgent || otherAgent.id === agent.id) continue;

          const allTrades = await db.query.trades.findMany();
          const tradesBetween = allTrades.filter(
            (t) =>
              (t.fromAgentId === agent.id && t.toAgentId === otherAgent.id) ||
...
apiRouter.get('/agents/:agentId/head-to-head/:otherAgentId', async (req, res) => {
  try {
    ...
    const tradesBetween = allTrades.filter(
      (t) =>
        (t.fromAgentId === agentId && t.toAgentId === otherAgentId) ||
        (t.fromAgentId === otherAgentId && t.toAgentId === agentId)
    );

    // Enrich trades with details
    const goods = await db.query.goods.findMany();
    const tradesHistory = await Promise.all(
      tradesBetween.map(async (trade) => {
