 ...
 */
function rotateAgentsByRound<T>(agents: T[], roundNumber: number): T[] {
...
export class RoundOrchestrator {
  ...
  private agentEngine: AgentExecutionEngine;
  ...
  private async executeAgentRound(
    roundId: string,
    agentId: string,
    modelId: string,
    provider: string
    ...
    const pendingProposals = await this.tradingService.getPendingProposals(
      agentId,
      roundId
    );
  ...
  private formatTimeRemaining(now: Date, endTime: Date): string {
