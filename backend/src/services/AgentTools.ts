...
export class AgentTools {
  ...
  async makeProposal(
    ...
    try {
      ...
      const inventory = await this.tradingService.getAgentInventory(
        this.sessionId,
        this.agentId,
        good.id
      );
   ...
   */
  async acceptProposal(proposalId: string, explanation: string): Promise<ToolResult> {
    try {
      ...
      const fromInventory = await this.tradingService.getAgentInventory(
        this.sessionId,
        proposal.fromAgentId,
        proposal.goodId
      );
      const toInventory = await this.tradingService.getAgentInventory(
        this.sessionId,
        proposal.toAgentId,
        proposal.goodId
      );
   ...
   */
  async rejectProposal(proposalId: string, explanation: string): Promise<ToolResult> {
    try {
      if (!proposalId || !explanation) {
        return {
          success: false,
   ...
   */
  async getAgentStates(): Promise<ToolResult> {
    try {
      // Fetch all agents
