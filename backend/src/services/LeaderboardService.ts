...
export class LeaderboardService {
   ...
   */
  async getDailyLeaderboard(sessionId: string): Promise<DailyLeaderboardEntry[]> {
   ...
   * Optimized with preloaded agent data to avoid N+1 queries
   */
  async getAllTimeLeaderboard(): Promise<AllTimeLeaderboardEntry[]> {
   ...
   */
  async calculateSessionLeaderboard(sessionId: string): Promise<void> {
    // Get all agents
    const agents = await db.query.agents.findMany();
