...
export class SessionScheduler {
  ...
  private roundOrchestrator: RoundOrchestrator | null = null;
  private tradingService: TradingService;
  private wsServer: WebSocketServer;
  private countdownInterval: NodeJS.Timeout | null = null;
  private currentSessionEndTime: Date | null = null;
  private isPaused: boolean = false;
  private sessionEndTimeout: NodeJS.Timeout | null = null;
  ...
  private async checkActiveSession() {
    ...
    if (activeSessions.length > 0) {
      ...
      this.roundOrchestrator = new RoundOrchestrator(
        session.id,
        this.wsServer
      );
  ...
  private async calculateLeaderboard(sessionId: string) {
  ...
  async getNextSessionTime(): Promise<Date> {
    const now = new Date();
    const [hour, minute] = TRADING_CONFIG.sessionStartTime.split(':').map(Number);
