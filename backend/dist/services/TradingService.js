...
export class TradingService {
    ...
    async counterProposal(originalProposalId, newProposal) {
    ...
    async settleTrade(params) {
        ...
        const fromAgentInventory = await this.getAgentInventory(sessionId, proposal.fromAgentId, proposal.goodId);
        const toAgentInventory = await this.getAgentInventory(sessionId, proposal.toAgentId, proposal.goodId);
        // 2. Validate sufficient resources
        if (fromAgentInventory.quantity < proposal.quantity) {
            throw new Error(`Agent ${proposal.fromAgentId} has insufficient ${proposal.goodId}`);
        }
        if (toAgentInventory.cashBalance < proposal.price) {
            throw new Error(`Agent ${proposal.toAgentId} has insufficient cash`);
        }
        ...
        await db
            ...
            .where(and(eq(schema.inventories.sessionId, sessionId), eq(schema.inventories.agentId, proposal.fromAgentId), eq(schema.inventories.goodId, proposal.goodId)));
        ...
        await db
            ...
            .where(and(eq(schema.inventories.sessionId, sessionId), eq(schema.inventories.agentId, proposal.fromAgentId)));
        ...
        await db
            ...
            .where(and(eq(schema.inventories.sessionId, sessionId), eq(schema.inventories.agentId, proposal.toAgentId), eq(schema.inventories.goodId, proposal.goodId)));
        ...
        await db
            ...
            .where(and(eq(schema.inventories.sessionId, sessionId), eq(schema.inventories.agentId, proposal.toAgentId)));
        // 4. Record trade
    ...
    async getAgentInventory(sessionId, agentId, goodId) {
        const inventory = await db.query.inventories.findFirst({
            where: and(eq(schema.inventories.sessionId, sessionId), eq(schema.inventories.agentId, agentId), eq(schema.inventories.goodId, goodId)),
        });
        if (!inventory) {
    ...
    async getAgentState(sessionId, agentId) {
        // Get all inventories for agent
        const inventories = await db.query.inventories.findMany({
            where: and(eq(schema.inventories.sessionId, sessionId), eq(schema.inventories.agentId, agentId)),
        });
        // Get goods info
        ...
        const trades = await db.query.trades.findMany({
            where: and(eq(schema.trades.sessionId, sessionId), or(eq(schema.trades.fromAgentId, agentId), // Trades where agent is seller
    ...
    async getPendingProposals(agentId, roundId) {
        const proposals = await db.query.proposals.findMany({
            where: and(eq(schema.proposals.toAgentId, agentId), eq(schema.proposals.roundId, roundId), eq(schema.proposals.status, 'pending')),
        });
    ...
    async initializeSessionInventories(sessionId) {
        ...
        for (const agent of agents) {
            for (const good of goods) {
                ...
                const quantity = INITIAL_INVENTORY[good.name] || 0;
